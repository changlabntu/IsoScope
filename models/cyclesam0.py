...
class AxialToLateralGANApolloModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # Code (vs. paper): G_A (G), G_B (F), D_A (D_Y), D_B (D_X)  # netG_A  bad > good
    ...
    def set_input(self, input): # setting inputs
    ...
    def forward(self):
        ...
        self.rec = self.netG_B(self.fake)  # G_B(G_A(A)) # for cyclic

    def backward_D_slice(self, netD, real, fake, slice_axis_real, slice_axis_fake): # slice_axis_real, slice_axis_fake for deciding the axis to slice
        ...
        # Real
        pred_real = self.iter_f(real, netD, slice_axis_real)  # real slice judged by netD
        pred_fake = self.iter_f(fake.detach(), netD, slice_axis_fake) # fake slice judged by netD
    ...
    def backward_D_projection(self, netD, real, fake, slice_axis_real, slice_axis_fake):  # ^^^ same but slice > projection
        ...
        # Real
        pred_real = self.iter_f(real, netD, slice_axis_real)   # real is only from slice
        pred_fake = self.proj_f(fake.detach(), netD, slice_axis_fake)  # fake is from projection
    ...
    def iter_f(self, input, function, slice_axis):  # this is to get slice and judged by the discriminator (function)
    ...
    def proj_f(self, input, function, slice_axis):  # this is to get projection and judged by the discriminator (function)
...
class Volume():
    ...
    def get_slice(self, slice_axis):  # VOLUME > SLICE (3D > 2D)
    ...
    def get_projection(self, depth, slice_axis):  # VOLUME > PROJECTION (3D > 2D)
