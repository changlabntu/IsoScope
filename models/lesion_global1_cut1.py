...
# python train.py --alpha 0 --jsn womac4 --prj global1_cut1/0/ --models lesion_global1_cut1 --netG edalphand --split moaks --dataset womac4 --lbvgg 0 --lbNCE 4 --nm 01 --fDown 4 --use_mlp --fWhich 0 0 1 1 -b 2 --ngf 32 --projection 32 --env runpod --n_epochs 200 --lr_policy cosine

def quick_gather():
    import glob, os
    import tifffile as tiff
    import numpy as np
    x_list = sorted(glob.glob('/media/ghc/Ghc_data3/OAI_diffusion_final/diffusion_classification/a/*'))
    for x in x_list[:]:
        name = x.split('/')[-1].split('.')[0]
        image_stacks = sorted(glob.glob('/media/ExtHDD01/Dataset/paired_images/womac4/full/seg/aseg/' + name + '*'))
        image_stacks = np.stack([tiff.imread(x) for x in image_stacks], 0)
        tiff.imwrite('/media/ghc/Ghc_data3/OAI_diffusion_final/diffusion_classification/aseg/' + name + '.tif', image_stacks)